            ...
            try:
                for message in consumer:
                    ...
                    try:
                        ...
                        logger.debug('going to write to the file')
                        fields=['first','second','third']
                        with open('./../output/ubikes.csv', 'a+') as f:
                            writer = csv.writer(f)
                            writer.writerow(fields)
                        logger.debug('wrote to the file')

                        logger.debug('encoded item')
                        if args['decode_base64'] and 'body' in item:
                            logger.debug('body is in item')
